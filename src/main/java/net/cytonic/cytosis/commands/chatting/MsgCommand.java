...
public class MsgCommand extends CytosisCommand {
    ...
    public MsgCommand() {
        ...
        playerArg.setSuggestionCallback(
            (sender, ctx, suggestion) -> Cytosis.getCytonicNetwork().getOnlinePlayers().getValues()
    ...
    private void sendMessage(CytosisPlayer actor, UUID recipient, String message) {
        ...
        PlayerRank targetRank = Cytosis.getCytonicNetwork().getCachedPlayerRanks().get(recipient);

        if (message.isEmpty()) {
            Component recipientComponent =
                targetRank.getPrefix()
                    .append(Component.text(Cytosis.getCytonicNetwork().getOnlinePlayers()
            ...
            Cytosis.getChatManager().openPrivateMessage(actor, recipient);
            return;
        }
