...
public class NickSetupCommand extends CytosisCommand {
    ...
    private static final Book WARNING_BOOK = Book.builder().author(Msg.aqua("CytonicMC Team"))
        .title(Msg.gold("Acknowledgements!")).pages(Msg.mm("""
                 <red>⚠ WARNING! ⚠</red>
            
            <black>All rules still apply. You can still be reported and all nickname history is stored. Staff can still
             see your true identity.</black>
    ...
    private static final Book SKIN_BOOK = Book.builder().author(Msg.aqua("CytonicMC Team"))
        .title(Msg.gold("Nickname Skin")).pages(Msg.mm("""
            <black>Great! Now, which <b><#F5C526>SKIN</#F5C526></b> you would like to use while nicked?
             <hover:show_text:'<gray><i>Your skin will appear to be your real skin, but the skin you chose will be
             shown to other players.'><gray><i>(note)</hover></black>
    ...
    private static final Book NAME_BOOK = Book.builder().author(Msg.aqua("CytonicMC Team"))
        .title(Msg.gold("Nickname Skin")).pages(Msg.mm("""
    ...
    public NickSetupCommand() {
        ...
        addSyntax((sender, context) -> {
            ...
            switch (context.get(verb).toLowerCase()) {
                ...
                case "rank" -> handleRank(player, arg);
                case "skin" -> handleSkin(player, arg);
                case "name" -> handleName(player, arg);
                default -> {
                }
            }
        }, verb, verbArg);
    ...
    private static Book getConfirmBook(CytosisPlayer player, NicknameManager.NicknameData data) {
        return Book.builder().author(Msg.aqua("CytonicMC Team")).title(Msg.gold("Confirm Details")).pages(Msg.mm("""
    ...
    private void handleRank(CytosisPlayer player, String arg) {
        if (!NICKNAME_DATA.containsKey(player.getUuid())) {
            player.sendMessage(NEED_TERMS);
            return;
        }
        if (arg.equals("not set")) {
            player.sendMessage(Msg.whoops("You must specify a rank!"));
            return;
        }
        PlayerRank rank;
        try {
            ...
            player.sendMessage(Msg.whoops("Invalid rank '%s'!", arg));
            return;
        }
    ...
    private void handleSkin(CytosisPlayer player, String arg) {
        if (!NICKNAME_DATA.containsKey(player.getUuid())) {
            player.sendMessage(NEED_TERMS);
            return;
        }
        if (arg.equals("not set")) {
            player.sendMessage(Msg.whoops("You must specify a skin option!"));
            return;
        }
        ...
        if (!SKIN_OPTIONS.contains(arg.toUpperCase())) {
            player.sendMessage(Msg.whoops("Invalid skin option '%s'!", arg));
            return;
        }
        switch (arg.toUpperCase()) {
            case "DEFAULT" -> NICKNAME_DATA.computeIfPresent(player.getUuid(), (k, data) -> data.withSkin(null, null));
    ...
    private void handleName(CytosisPlayer player, String arg) {
        if (!NICKNAME_DATA.containsKey(player.getUuid())) {
            player.sendMessage(NEED_TERMS);
            return;
        }
        if (arg.equals("not set")) {
            player.sendMessage(Msg.whoops("You must specify a name option!"));
            return;
        }

        if (!NAME_OPTIONS.contains(arg.toUpperCase())) {
            player.sendMessage(Msg.whoops("Invalid name option '%s'!", arg));
            return;
        }
        switch (arg.toUpperCase()) {
            case "SET" -> {
            ...
            case "SKIP" -> {
                if (NICKNAME_DATA.get(player.getUuid()).nickname() == null || NICKNAME_DATA.get(
                    player.getUuid()).nickname().isBlank()) {
                    player.sendMessage(Msg.whoops("You must set a nickname!"));
                    return;
                }
            }
            case "RANDOM" -> NICKNAME_DATA.computeIfPresent(player.getUuid(),
