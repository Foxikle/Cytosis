...
public class SnooperProvider extends ViewProvider {
    ...
    public SnooperProvider(@NotNull final String id, String search, boolean ascending) {
        super(Cytosis.VIEW_REGISTRY,
            ViewBuilder.of(ViewType.SIZE_6_X_9).disableClickTypes(Click.Double.class).title(id));
    ...
    public PrebuiltItem toggleOrder() {
        return PrebuiltItem.of(
            ItemStack.builder(Material.EYE_ARMOR_TRIM_SMITHING_TEMPLATE).customName(Msg.mm("<yellow>Change Order"))
                .lore(Msg.mm("<b>" + (ascending ? "<green>Ascending" : "<red>Descending")), Component.empty(),
                    Msg.mm("<yellow>Click to toggle")).hideExtraTooltip().build(), action -> {
                ascending = !ascending;
    ...
    public PrebuiltItem dateRange() {
        ...
        return PrebuiltItem.of(
            ItemStack.builder(Material.CLOCK).lore(lore).customName(Msg.mm("<yellow>Change Date Range"))
                .hideExtraTooltip().build(), action -> {
                action.getEvent().setCancelled(true);
    ...
    private void fetchData(PlayerView view) {
        pattern.getSlots('1').forEach(slot -> ViewItem.bySlot(view, slot).applyPrebuilt(
            PrebuiltItem.empty().material(Material.LIGHT_GRAY_STAINED_GLASS_PANE).name("<gray>Loading data!")
                .cancelClicking()));
        ...
        Cytosis.getSnooperManager().getPersistenceManager()
            ...
            .thenAccept(queriedSnoops -> {
                ...
                if (error != null) {
                    ViewItem.bySlot(view, 22).applyPrebuilt(
                        PrebuiltItem.empty().material(Material.RED_STAINED_GLASS_PANE).name("<red>Failed to load data!")
                            .cancelClicking());
    ...
    /**
     * Creates a new {@link PlayerView} for the given {@link Player} and opens it.
     *
    ...
    private static class SnooperRenderer implements ItemRenderer<QueriedSnoop> {
        ...
        public PrebuiltItem render(QueriedSnoop snoop) {
            ...
            lore.add(Msg.mm(""));
            lore.add(Msg.mm("<yellow>Sent: <light_purple>" + DurationParser.unparseFull(snoop.timestamp().toInstant())
                + "</light_purple> ago."));

            ItemStack item = ItemStack.builder(Material.PAPER).hideExtraTooltip()
                .customName(Msg.mm("Snoop #" + snoop.id())).lore(lore).build();
